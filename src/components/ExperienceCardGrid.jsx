...
const experienceTypeColors = {
  "Intern": "bg-indigo-100 text-indigo-700 border-indigo-300",
...
export function ExperienceCardGrid({ experience, onSave, isSaved, onClick, isLoggedIn = true, onLoginRequired }) {
  const companyColor = getColorFromString(experience.company || "");
  const expType = experience.experienceType || experience.type;
  
  return (
    <div 
      className="bg-white rounded-xl border border-gray-200 p-6 hover:shadow-xl hover:border-purple-200 transition-all duration-300 cursor-pointer group"
      onClick={onClick}
    >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-4 mb-2">
            <div className="flex-1">
              <h3 className="text-gray-900 font-semibold text-lg group-hover:text-purple-600 transition-colors">{experience.title}</h3>
            </div>
            <Badge variant="outline" className={`flex-shrink-0 ${experienceTypeColors[expType] || "bg-gray-100 text-gray-700 border-gray-300"}`}>
              {expType}
            </Badge>
          </div>
          ...
          {experience.duration && (
            <p className="text-gray-600 mb-3 flex items-center">
              <span className="mr-1">⏱️</span> Duration: {experience.duration}
            </p>
          )}
          ...
          <div className="flex items-center justify-between pt-3 border-t border-gray-100">
            <p className="text-gray-500 text-sm">📅 Posted: {experience.postedDate}</p>
            <Button
              variant="outline"
              size="sm"
              onClick={(e) => {
